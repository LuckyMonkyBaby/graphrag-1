...
try:
    ...
    log.warning(
        "pdfplumber not installed. PDF parsing will not be available. Install with: pip install pdfplumber"
    )
...
try:
    ...
    log.warning(
        "PyPDF2 not installed. Fallback PDF parsing will not be available. Install with: pip install PyPDF2"
    )
    ...
    async def load_file(path: str, group: Optional[Dict] = None) -> pd.DataFrame:
        ...
        try:
            ...
            if PDF_PLUMBER_AVAILABLE:
                document_structure = extract_pdf_structure_pdfplumber(
                    pdf_bytes, Path(path).name
                )
            elif PYPDF2_AVAILABLE:
                document_structure = extract_pdf_structure_pypdf2(
                    pdf_bytes, Path(path).name
                )
            else:
        ...
        log.info(
            f"Created PDF document entry with ID: {new_item['id']}, Title: {new_item['title']}"
        )
...
def extract_pdf_structure_pdfplumber(pdf_bytes: bytes, filename: str) -> Dict[str, Any]:
    ...
    log.info(
        f"Extracted {len(document_structure['pages'])} pages and {len(document_structure['paragraphs'])} paragraphs"
    )
