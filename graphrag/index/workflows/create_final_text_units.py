...
def create_final_text_units(
    ...
    """All the steps to transform the text units."""
    log.info(
        f"Creating final text units from {len(text_units) if text_units is not None else 'None'} input rows"
    )

    # Select basic columns from text_units
    basic_columns = ["id", "text", "document_ids", "n_tokens"]
    selected = text_units.loc[
        :, [col for col in basic_columns if col in text_units.columns]
    ].copy()
    log.info(
        f"Selected {len(selected)} rows with basic columns: {[c for c in basic_columns if c in text_units.columns]}"
    )
    ...
    for col in html_columns:
        if col in text_units.columns:
            ...
            complex_objects = (
                text_units[col].apply(lambda x: isinstance(x, (dict, list))).sum()
            )
            if complex_objects > 0:
                log.warning(
                    f"Found {complex_objects} complex objects in column {col}, converting to strings"
                )
    ...
    for col in final_joined.columns:
        ...
        sample_value = (
            final_joined[col].dropna().iloc[0]
            if not final_joined[col].dropna().empty
            else None
        )
        log.debug(
            f"Column {col}: dtype={final_joined[col].dtype}, sample value type={type(sample_value)}"
        )
    ...
    for col in final_joined.columns:
        if col.endswith("_ids"):
            ...
            non_list_count = (
                final_joined[col]
                .apply(lambda x: not isinstance(x, list) and x is not None)
                .sum()
            )

            if non_list_count > 0:
                log.warning(
                    f"Found {non_list_count} non-list values in column {col}, converting to lists"
                )
    ...
    missing_columns = [
        col for col in TEXT_UNITS_FINAL_COLUMNS if col not in aggregated.columns
    ]
    if missing_columns:
    ...
    for col in ["document_ids", "entity_ids", "relationship_ids", "covariate_ids"]:
        if col in aggregated.columns:
            ...
            mixed_types = (
                aggregated[col]
                .apply(lambda x: not (x is None or isinstance(x, list)))
                .sum()
            )

            if mixed_types > 0:
                log.warning(
                    f"Found {mixed_types} mixed type values in {col} during final check"
                )
    ...
    if "children" in aggregated.columns:
        ...
        complex_objects = (
            ...
            .apply(
                lambda x: not (
                    ...
                    or isinstance(x, (str, int, float, bool))
                )
            )
            .sum()
        )
        ...
        aggregated["children"] = aggregated["children"].apply(
            lambda x: [] if x is None else (x if isinstance(x, list) else [str(x)])
        )
    ...
    for col in aggregated.columns:
        if (
            ...
            complex_count = (
                aggregated[col].apply(lambda x: isinstance(x, (dict, list))).sum()
            )

            if complex_count > 0:
                log.warning(
                    f"Found {complex_count} complex objects in column {col}, converting to strings"
                )
    ...
    for col in TEXT_UNITS_FINAL_COLUMNS:
        if col in aggregated.columns:
            sample_value = (
                aggregated[col].dropna().iloc[0]
                if not aggregated[col].dropna().empty
                else None
            )
            log.debug(
                f"Column {col}: dtype={aggregated[col].dtype}, sample value type={type(sample_value)}"
            )

    # Return only the specified columns to avoid any extra problematic columns
    result = aggregated.loc[:, TEXT_UNITS_FINAL_COLUMNS]
    log.info(
        f"Final result: {len(result)} rows with {len(TEXT_UNITS_FINAL_COLUMNS)} columns"
    )
...
def simplify_attributes(attrs):
    ...
    if isinstance(attrs, str):
        try:
            log.debug(
                f"Parsing attributes JSON string: {attrs[:100]}..."
                if len(attrs) > 100
                else attrs
            )
        ...
        for key, value in html.items():
            if isinstance(value, (str, int, float, bool, type(None))):
                ...
                if isinstance(value, list):
                    log.info(
                        f"PRESERVED PRIMARY structural array '{key}' with {len(value)} elements"
                    )
                else:
                    log.info(
                        f"PRESERVED PRIMARY structural data '{key}': {type(value)}"
                    )
            else:
                ...
                try:
                    ...
                    log.debug(
                        f"Converted non-serializable HTML property {key} to string"
                    )
    ...
    log.info(
        f"Attributes processed with PRIMARY structural data preserved: {preserved_keys}"
    )
...
def _entities(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .agg(
            entity_ids=("id", lambda x: list(pd.Series(x).unique()))
        )  # Ensure list output
        .reset_index()
...
def _relationships(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        unrolled.groupby("text_unit_ids", sort=False)
        .agg(
            relationship_ids=("id", lambda x: list(pd.Series(x).unique()))
        )  # Ensure list output
        .reset_index()
        .rename(columns={"text_unit_ids": "id"})
    )
...
def _covariates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .agg(
            covariate_ids=("id", lambda x: list(pd.Series(x).unique()))
        )  # Ensure list output
        .reset_index()
