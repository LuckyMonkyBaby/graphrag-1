...
class AWSConfig(BaseModel):
    ...
    dynamodb_table_prefix: str = Field(
        default="graphrag_", description="DynamoDB table prefix"
    )

    # OpenSearch Configuration
    opensearch_domain: Optional[str] = Field(
        default=None, description="OpenSearch domain endpoint"
    )
    opensearch_index_prefix: str = Field(
        default="graphrag_", description="OpenSearch index prefix"
    )

    # Neptune Configuration (for graph storage)
    neptune_cluster_endpoint: Optional[str] = Field(
        default=None, description="Neptune cluster endpoint"
    )
    neptune_reader_endpoint: Optional[str] = Field(
        default=None, description="Neptune reader endpoint"
    )

    # RDS Configuration (for metadata)
    rds_endpoint: Optional[str] = Field(
        default=None, description="RDS endpoint for metadata"
    )
...
class AzureConfig(BaseModel):
    ...
    storage_container: str = Field(
        default="graphrag", description="Blob container name"
    )

    # Cosmos DB Configuration
    cosmos_account_name: Optional[str] = Field(
        default=None, description="Cosmos DB account name"
    )
    cosmos_database: str = Field(
        default="graphrag", description="Cosmos DB database name"
    )

    # AI Search Configuration
    search_service_name: Optional[str] = Field(
        default=None, description="Azure AI Search service"
    )
    search_index_prefix: str = Field(
        default="graphrag_", description="Search index prefix"
    )
...
class LanceDBConfig(BaseModel):
    ...
    storage_path: str = Field(
        default="./lancedb", description="Local path for LanceDB storage"
    )
    s3_bucket: Optional[str] = Field(
        default=None, description="S3 bucket for LanceDB cloud storage"
    )
    s3_region: Optional[str] = Field(default=None, description="S3 region for LanceDB")

    # Performance configuration
    vector_column_name: str = Field(
        default="vector", description="Name of vector column"
    )
    ...
    relationships_table: str = Field(
        default="relationships", description="Relationships table name"
    )
    text_units_table: str = Field(
        default="text_units", description="Text units table name"
    )
    embeddings_table: str = Field(
        default="embeddings", description="Embeddings table name"
    )

    # Connection configuration
    read_consistency_level: str = Field(
        default="eventual", description="Read consistency level"
    )
...
class CloudStorageConfig(BaseModel):
    ...
    provider: CloudProvider = Field(
        default=CloudProvider.LOCAL, description="Cloud provider"
    )

    # Storage backends
    document_storage: StorageBackend = Field(
        default=StorageBackend.FILE_SYSTEM, description="Document storage backend"
    )
    metadata_storage: StorageBackend = Field(
        default=StorageBackend.SQLITE, description="Metadata storage backend"
    )
    vector_storage: VectorStoreBackend = Field(
        default=VectorStoreBackend.LANCEDB, description="Vector storage backend"
    )
    graph_storage: GraphStoreBackend = Field(
        default=GraphStoreBackend.NETWORKX, description="Graph storage backend"
    )
    ...
    lancedb: LanceDBConfig = Field(
        default_factory=LanceDBConfig, description="LanceDB configuration"
    )
...
class MultiCloudConfig(BaseModel):
    ...
    primary_provider: CloudProvider = Field(
        default=CloudProvider.LOCAL, description="Primary cloud provider"
    )
    failover_provider: Optional[CloudProvider] = Field(
        default=None, description="Failover cloud provider"
    )

    # Cross-cloud replication
    enable_cross_cloud_backup: bool = Field(
        default=False, description="Enable cross-cloud backup"
    )
    backup_providers: list[CloudProvider] = Field(
        default_factory=list, description="Backup cloud providers"
    )

    # Load balancing
    enable_load_balancing: bool = Field(
        default=False, description="Enable multi-cloud load balancing"
    )
    ...
    # Data locality
    prefer_local_storage: bool = Field(
        default=True, description="Prefer storage close to compute"
    )
    max_cross_region_latency_ms: int = Field(
        default=100, description="Max acceptable cross-region latency"
    )
