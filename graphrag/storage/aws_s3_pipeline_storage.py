...
class S3PipelineStorage(PipelineStorage):
    ...
    async def _get_s3_client(self):
        ...
        if self._s3_client is None:
            try:
                ...
                log.info(f"Connected to S3 bucket: {self.bucket} in region: {self.region}")
    ...
    async def get(self, path: str, encoding: Optional[str] = None, as_bytes: bool = False) -> Any:
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: s3_client.get_object(Bucket=self.bucket, Key=s3_key)
            )
    ...
    async def has(self, path: str) -> bool:
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: s3_client.head_object(Bucket=self.bucket, Key=s3_key)
            )
            return True
    ...
    async def delete(self, path: str) -> None:
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: s3_client.delete_object(Bucket=self.bucket, Key=s3_key)
            )
    ...
    async def copy_file(self, source_path: str, dest_path: str) -> None:
        ...
        try:
            ...
            log.error(f"Failed to copy S3 object from {source_path} to {dest_path}: {e}")
            raise
